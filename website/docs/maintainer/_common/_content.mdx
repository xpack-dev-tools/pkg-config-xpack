...
**@xpack-dev-tools/pkg-config** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
```sh
rm -rf ~/Work/xpack-dev-tools/pkg-config-xpack.git/package-lock.json
```

```sh
rm -rf ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/pkg-config-xpack.git pull

npm --prefix ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets
...
(such as **0.29.2-3.1**) with the following expression:
...
xpm run trigger-workflow-test-docker-linux-arm64 -C ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets
```
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v0.29.2-3 released**_
...
tagged such as **v0.29.2-3** (mind the dash in the middle!) and
named such as **xPack pkg-config v0.29.2-3** (mind the dash),
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/pkg-config-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/pkg-config-xpack.git/build-assets
```
