...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fpkg-config?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/pkg-config/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/pkg-config-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fpkg-config?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/pkg-config/)

The **xPack pkg-config** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install pkg-config
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/pkg-config`}><code>@xpack-dev-tools/pkg-config</code></a> from
...
<code>xpacks/@xpack-dev-tools/pkg-config</code>.
...
**pkg-config** package into the project.

The command to install the **latest** available version of **pkg-config** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/pkg-config@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of pkg-config, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/pkg-config</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/pkg-config/.content/bin</code>;
* add <code>@xpack-dev-tools/pkg-config</code> to
  `package.json` as a
  development dependency; this associates a specific version of pkg-config with
...
it is also possible to install pkg-config only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/pkg-config@latest --global --verbose
`} </CodeBlock>
...
In order to access the pkg-config binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/pkg-config/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/pkg-config/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/pkg-config --verbose
`} </CodeBlock>
...
the <a href={`https://github.com/xpack-dev-tools/pkg-config-xpack/releases`}>GitHub Releases</a> pages.
...
<ul>
<li><code>xpack-pkg-config-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-pkg-config-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
To manually install the <b>xPack pkg-config</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>~/Library/xPacks/pkg-config</code> folder, for
example <code>/Users/ilg/Library/xPacks/pkg-config/xpack-pkg-config-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/pkg-config
cd ~/Library/xPacks/pkg-config

tar xvf ~/Downloads/xpack-pkg-config-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-pkg-config-${customField('xpackVersion')}
`} </CodeBlock>
...
<ul>
<li><code>xpack-pkg-config-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-pkg-config-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-pkg-config-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/.local/xPacks/pkg-config</code> folder, for
example <code>/home/ilg/.local/xPacks/pkg-config/xpack-pkg-config-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/pkg-config
cd ~/.local/xPacks/pkg-config

tar xvf ~/Downloads/xpack-pkg-config-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-pkg-config-${customField('xpackVersion')}
`} </CodeBlock>
