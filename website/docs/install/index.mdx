...
The easiest (and recommended) way to install xPack pkg-config
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/pkg-config@0.29.2-3.1 --verbose
`} </CodeBlock>
...
* add <code>@xpack-dev-tools/pkg-config</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack pkg-config with
...
it is also possible to install xPack pkg-config only globally in
...
In order to access the xPack pkg-config binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/pkg-config/0.29.2-3.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/pkg-config/0.29.2-3.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-pkg-config-0.29.2-3-darwin-x64.tar.gz</code></li>
<li><code>xpack-pkg-config-0.29.2-3-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/pkg-config/xpack-pkg-config-0.29.2-3</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/pkg-config
cd ~/Library/xPacks/pkg-config

tar xvf ~/Downloads/xpack-pkg-config-0.29.2-3-darwin-x64.tar.gz
chmod -R -w xpack-pkg-config-0.29.2-3
`} </CodeBlock>
...
<ul>
<li><code>xpack-pkg-config-0.29.2-3-linux-x64.tar.gz</code></li>
<li><code>xpack-pkg-config-0.29.2-3-linux-arm64.tar.gz</code></li>
<li><code>xpack-pkg-config-0.29.2-3-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/pkg-config/xpack-pkg-config-0.29.2-3</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/pkg-config
cd ~/.local/xPacks/pkg-config

tar xvf ~/Downloads/xpack-pkg-config-0.29.2-3-linux-x64.tar.gz
chmod -R -w xpack-pkg-config-0.29.2-3
`} </CodeBlock>
